    ...
    with open(filename, 'w', newline='') as file:
        ...
        if(searchpath == "no solution"):
            ...
            for node in searchpath:
                f_score = node[0]
                h_score = get_h_score(heuristics_option, node[1])
...
def filter_invalid_moves(explored_list, init, goalstate):
    ...
    while currentn != goalstate:
        if(i + 1 > (len(explored_list) - 1)):
            #do something
            #print("Dead End found at: ", currentn)
...
def a_star_search(board, pmfilename, spfilename, i):
...
def main():
    ...
    for brds in board:
        ...
        a_star_search(brds, pathmovefilename, searchpathfilename, i)
        i += 1
